...
from projects.polymarket.polyquantbot.platform.execution.wallet_capital import (
    WALLET_CAPITAL_BLOCK_MONITORING_ANOMALY,
    WALLET_CAPITAL_HALT_MONITORING_ANOMALY,
...
VALID_MONITORING_INPUT = MonitoringContractInput(
    policy_ref="phase6_4_7_policy",
    eval_ref="phase6_4_7_eval",
    timestamp_ms=1713207000000,
    exposure_ratio=0.10,
    ...
    trace_refs={"target_path": "wallet_capital.authorize_capital_with_trace"},
)
...
VALID_SIGNING_RESULT = SigningResult(
    ...
    success=True,
    blocked_reason=None,
    signature="sig-6-4-7-001",
    payload_hash="hash-6-4-7-001",
...
def _wallet_policy() -> WalletCapitalPolicyInput:
    return WalletCapitalPolicyInput(
        ...
        policy_trace_refs={"policy": "phase6.4.7.capital"},
    )
...
def _wallet_execution_input() -> WalletCapitalExecutionInput:
    return WalletCapitalExecutionInput(
        ...
        upstream_trace_refs={"phase": "6.4.7"},
    )


def test_phase6_4_7_capital_monitoring_allow_pass_through() -> None:
...
def test_phase6_4_7_capital_monitoring_block_prevents_authorization() -> None:
    controller = WalletCapitalController(real_capital_enabled=True)
    breaker = MonitoringCircuitBreaker()

    build = controller.authorize_capital_with_trace(
        execution_input=replace(
            _wallet_execution_input(),
    ...
    assert build.result.blocked_reason == WALLET_CAPITAL_BLOCK_MONITORING_ANOMALY
...
def test_phase6_4_7_capital_monitoring_halt_stops_authorization() -> None:
    controller = WalletCapitalController(real_capital_enabled=True)
    breaker = MonitoringCircuitBreaker()
    ...
    assert build.result is not None
    assert build.result.capital_authorized is False
    assert build.result.blocked_reason == WALLET_CAPITAL_HALT_MONITORING_ANOMALY
    assert build.trace.upstream_trace_refs["monitoring"]["primary_anomaly"] == ANOMALY_KILL_SWITCH_TRIGGERED
    assert len(breaker.get_events()) == 1


def test_phase6_4_7_existing_five_monitored_paths_remain_intact() -> None:
    ...
    signing_build = SecureSigningEngine(real_signing_enabled=True).sign_with_trace(
        signing_input=SigningExecutionInput(
            exchange_result=ExchangeExecutionResult(
                executed=True,
                success=True,
                blocked_reason=None,
                execution_id="EXE-6-4-7-001",
                request_payload={"execution_id": "EXE-6-4-7-001"},
                signed_payload={"execution_id": "EXE-6-4-7-001"},
