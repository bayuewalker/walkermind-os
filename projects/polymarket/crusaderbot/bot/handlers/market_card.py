...
def _build_market_card(
    ...
    *,
    ...
    if signal_type or strategy_name or confidence_pct is not None:
        lines.append("")
        if signal_type:
            strat_str = f" ({_escape_md(strategy_name)})" if strategy_name else ""
            lines.append(f"Signal: {_escape_md(signal_type)}{strat_str}")
