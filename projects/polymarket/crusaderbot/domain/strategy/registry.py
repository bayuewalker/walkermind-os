...
def bootstrap_default_strategies(
    ...
    tests after `_reset_for_tests()`. Imports happen inside the function to
    avoid an import-time cycle between `registry` and `strategies/`.
    """
    ...
    for cls in (CopyTradeStrategy,):
        if cls.name in reg._strategies:
            continue
        reg.register(cls())
    return reg
