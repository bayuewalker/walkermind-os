...
def test_already_mirrored_scopes_query_per_follower():
    ...
    with patch.object(ct_mod, "get_pool", return_value=_CapturingPool()):
        out = asyncio.run(
            ct_mod._already_mirrored(str(_USER_UUID), _TARGET_UUID, "0x" + "a" * 64),
        )
    assert out is False
...
def test_already_mirrored_returns_false_on_blank_inputs():
    ...
    out = asyncio.run(ct_mod._already_mirrored(None, _TARGET_UUID, "0xabc"))
    assert out is False
    out = asyncio.run(ct_mod._already_mirrored(str(_USER_UUID), _TARGET_UUID, ""))
    assert out is False
...
def _build_target_row():
    return {
        ...
        "user_id": _USER_UUID,
        "wallet_address": "0x" + "a" * 40,
        "task_name": "Regression Target",
        "copy_mode": "proportional",
        "copy_amount": None,
        "copy_pct": 0,  # zero pct triggers mirror_size_direct fallback
        "copy_direction": "buys_only",
        "min_trade_size": 0.50,
        "created_at": datetime.now(timezone.utc),
    }
...
# ---------------------------------------------------------------------------
# Monitor filter tests (migration 035): copy_direction, allow_topups,
# execution_mode. All tests exercise _process_one() directly via the
# monitor module's internal function, mocking DB and event_bus.
# ---------------------------------------------------------------------------
...
def _make_leader_trade(side: str = "BUY", usdc_size: float = 20.0) -> dict:
    """Minimal leader trade dict as returned by wallet_watcher."""
    from datetime import datetime, timezone
...
def test_monitor_execution_mode_manual_emits_event_not_execute():
    ...
    assert emitted[0]["event_type"] == "copy_trade.pending_confirm"
    assert emitted[0]["telegram_user_id"] == 99999
