Last Updated : 2026-05-09 10:30 Asia/Jakarta
Status       : Phase 4A CLOB Adapter (WARP/CRUSADERBOT-PHASE4A-CLOB-ADAPTER) SENTINEL APPROVED 89/100. New integrations/clob/* package: ClobAdapter (REST against clob.polymarket.com with EIP-712 L1 + HMAC-SHA256 L2 + optional builder headers, tenacity retry, no-retry on 4xx, typed ClobAuthError/ClobAPIError/ClobConfigError), MarketDataClient (unauthenticated reads — orderbook, midpoint, spread, price, market, tick-size, neg-risk), MockClobClient (deterministic, network-free, asserts no httpx.AsyncClient instantiated), get_clob_client() factory toggled by USE_REAL_CLOB env (default False = paper-safe; True with missing creds raises ClobConfigError, no silent fallback). Six new optional Settings fields (POLYMARKET_API_PASSPHRASE alias, POLYMARKET_PRIVATE_KEY, POLYMARKET_FUNDER_ADDRESS, POLYMARKET_SIGNATURE_TYPE default 2, three POLYMARKET_BUILDER_* fields) plus USE_REAL_CLOB flag — all additive. 45 new unit tests (auth EIP-712 + HMAC vectors locked; adapter/market-data/factory/mock paths) + 1 manual integration smoke file (CLOB_SMOKE=1 env-gated, never auto-runs in CI). 624/624 crusaderbot tests green (was 556). Ruff clean on every changed file. Live execution callers (domain.execution.live) NOT yet rewired — Phase 4B scope. Activation guards remain NOT SET. Earlier lanes: Lane 1C (CRU-5) PR #908 SENTINEL APPROVED 98/100 awaiting merge; Ops dashboard + Tier 2 operator seed (STANDARD) forge complete awaiting WARP🔹CMD review.
...
[IN PROGRESS]
- Phase 4A CLOB Adapter — WARP/CRUSADERBOT-PHASE4A-CLOB-ADAPTER. Tier MAJOR / Claim NARROW INTEGRATION. New integrations/clob/ package: ClobAdapter (EIP-712 L1 + HMAC-SHA256 L2 + builder headers; tenacity retry on 5xx only; 4xx surfaces typed ClobAuthError / ClobAPIError without retry — capital-safety: no duplicate POST after broker-class reject), MarketDataClient (unauth reads, separate transport, never signs), MockClobClient (deterministic, network-free), get_clob_client() factory toggled by USE_REAL_CLOB env (default False = paper-safe). Six new optional Polymarket Settings fields + USE_REAL_CLOB. 45 new unit tests with mocked httpx.MockTransport + locked EIP-712 / HMAC test vectors; 1 integration smoke file (CLOB_SMOKE env-gated, skipped in CI). 624/624 green (was 556). Ruff clean. py-clob-client.OrderBuilder still owns on-chain order schema (CTF Exchange EIP-712) — Phase 4C scope. Live execution callers (domain.execution.live) NOT rewired in this lane — Phase 4B. Activation guards remain NOT SET. Forge report: projects/polymarket/crusaderbot/reports/forge/clob-adapter.md. SENTINEL APPROVED 89/100 — awaiting WARP🔹CMD merge decision. Sentinel report: projects/polymarket/crusaderbot/reports/sentinel/clob-adapter.md.
...
[NEXT PRIORITY]
- WARP🔹CMD merge decision on WARP/CRUSADERBOT-PHASE4A-CLOB-ADAPTER (MAJOR / NARROW INTEGRATION, SENTINEL APPROVED 89/100). Report: projects/polymarket/crusaderbot/reports/sentinel/clob-adapter.md.
