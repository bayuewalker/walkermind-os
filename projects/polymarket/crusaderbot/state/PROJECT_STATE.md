...
[IN PROGRESS]
- Phase 4A CLOB Adapter — WARP/CRUSADERBOT-PHASE4A-CLOB-ADAPTER. Tier MAJOR / Claim NARROW INTEGRATION / SENTINEL REQUIRED. New integrations/clob/ package: ClobAdapter (EIP-712 L1 + HMAC-SHA256 L2 + builder headers; tenacity retry on 5xx only; 4xx surfaces typed ClobAuthError / ClobAPIError without retry — capital-safety: no duplicate POST after broker-class reject), MarketDataClient (unauth reads, separate transport, never signs), MockClobClient (deterministic, network-free), get_clob_client() factory toggled by USE_REAL_CLOB env (default False = paper-safe). Six new optional Polymarket Settings fields + USE_REAL_CLOB. 45 new unit tests with mocked httpx.MockTransport + locked EIP-712 / HMAC test vectors; 1 integration smoke file (CLOB_SMOKE env-gated, skipped in CI). 624/624 green (was 556). Ruff clean. py-clob-client.OrderBuilder still owns on-chain order schema (CTF Exchange EIP-712) — Phase 4C scope. Live execution callers (domain.execution.live) NOT rewired in this lane — Phase 4B. Activation guards remain NOT SET. Forge report: projects/polymarket/crusaderbot/reports/forge/clob-adapter.md. Awaiting WARP•SENTINEL audit before merge.
...
[NEXT PRIORITY]
- WARP•SENTINEL validation required for Phase 4A CLOB Adapter before merge. Source: projects/polymarket/crusaderbot/reports/forge/clob-adapter.md. Tier: MAJOR.
