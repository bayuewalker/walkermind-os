Last Updated : 2026-05-09 14:30 Asia/Jakarta
Status       : Phase 4B Execution Rewire (WARP/CRUSADERBOT-PHASE4B-EXECUTION-REWIRE) FORGE COMPLETE — SENTINEL REQUIRED before merge. domain/execution/live.py migrated off legacy py-clob-client SDK path onto get_clob_client() factory / ClobClientProtocol (Phase 4A). execute() wired: dry-run mode (USE_REAL_CLOB=True + ENABLE_LIVE_TRADING=False → log intent, return mock fill, no DB/broker), all 3 env guards still in assert_live_guards(), ClobAuthError→LivePreSubmitError / other→LivePostSubmitError, order_type GTC+FOK via post_order, per-order idempotency_key with ON CONFLICT DO NOTHING. close_position() uses client.post_order(side=SELL, order_type=GTC). 19 new unit tests hermetic (no DB/network/Telegram). Ruff clean. Activation guards remain NOT SET. 624+19=643 expected tests green in CI. Phase 4A CLOB Adapter still awaiting WARP🔹CMD merge decision (SENTINEL APPROVED 89/100).
...
[IN PROGRESS]
- Phase 4B Execution Rewire — WARP/CRUSADERBOT-PHASE4B-EXECUTION-REWIRE. Tier MAJOR / Claim NARROW INTEGRATION. domain/execution/live.py migrated off py-clob-client SDK onto get_clob_client() / ClobClientProtocol. Dry-run mode, guard routing, GTC/FOK, idempotency, ClobAuthError pre-submit classification. 19 new hermetic unit tests. Forge report: projects/polymarket/crusaderbot/reports/forge/execution-rewire.md. SENTINEL REQUIRED before merge.
- Phase 4A CLOB Adapter — WARP/CRUSADERBOT-PHASE4A-CLOB-ADAPTER. Tier MAJOR / Claim NARROW INTEGRATION. New integrations/clob/ package: ClobAdapter (EIP-712 L1 + HMAC-SHA256 L2 + builder headers; tenacity retry on 5xx only; 4xx surfaces typed ClobAuthError / ClobAPIError without retry — capital-safety: no duplicate POST after broker-class reject), MarketDataClient (unauth reads, separate transport, never signs), MockClobClient (deterministic, network-free), get_clob_client() factory toggled by USE_REAL_CLOB env (default False = paper-safe). Six new optional Polymarket Settings fields + USE_REAL_CLOB. 45 new unit tests with mocked httpx.MockTransport + locked EIP-712 / HMAC test vectors; 1 integration smoke file (CLOB_SMOKE env-gated, skipped in CI). 624/624 green (was 556). Ruff clean. py-clob-client.OrderBuilder still owns on-chain order schema (CTF Exchange EIP-712) — Phase 4C scope. Activation guards remain NOT SET. Forge report: projects/polymarket/crusaderbot/reports/forge/clob-adapter.md. SENTINEL APPROVED 89/100 — awaiting WARP🔹CMD merge decision. Sentinel report: projects/polymarket/crusaderbot/reports/sentinel/clob-adapter.md.
...
[NEXT PRIORITY]
- WARP•SENTINEL validation required for Phase 4B Execution Rewire (WARP/CRUSADERBOT-PHASE4B-EXECUTION-REWIRE) before merge. Source: projects/polymarket/crusaderbot/reports/forge/execution-rewire.md. Tier: MAJOR.
