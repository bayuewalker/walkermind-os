...
Status       : role-model-admin-user PR open (WARP/role-model-admin-user, MAJOR). Two-role model (admin + user): paper opened to all users, operator/tier/allowlist/premium user-facing wording removed → "Admin access required." / "This feature is not available."; internal Tier-4 + activation-guard live-safety gate intentionally preserved. Production PAPER ONLY.
...
[NEXT PRIORITY]
- WARP•SENTINEL validation required for role-model-admin-user (MAJOR) before merge. Source: projects/polymarket/crusaderbot/reports/forge/role-model-admin-user.md. Tier: MAJOR. Validates: paper-open change does not weaken any LIVE safety path; assert_live_guards (Tier-4 + activation guards) intact; risk-gate step-3 still blocks live for tier<3; no production handler relies on removed tier wording.
