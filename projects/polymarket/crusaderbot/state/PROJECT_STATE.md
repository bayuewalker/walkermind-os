...
Status       : R12d Telegram position UX merged (PR #868) — live position monitor (📈 Positions menu) with mark price + unrealized P&L + per-position [🛑 Force Close] confirm dialog wired to force_close_intent marker (Tier 2 view / Tier 3 close). R12c exit watcher PR still open: WARP/CRUSADERBOT-R12C-EXIT-WATCHER — MAJOR tier, SENTINEL audit required before merge. R12a CI/CD pipeline PR still open: WARP/CRUSADERBOT-R12A-CICD-PIPELINE. Paper-default.
...
- PR #868 — feat(crusaderbot): R12d Telegram position UX — live monitor + per-position force close (STANDARD, merged)
