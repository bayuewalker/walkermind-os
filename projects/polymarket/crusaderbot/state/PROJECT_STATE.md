...
[NEXT PRIORITY]
- WARP•SENTINEL validation required for crusaderbot-finalize (MAJOR) before merge. Source: projects/polymarket/crusaderbot/reports/forge/crusaderbot-finalize.md. Tier: MAJOR. Validates: ENABLE_LIVE_TRADING default flip (paper-safe posture), risk/guard layer unaffected, copy-task P&L tenant scoping, notifications fail-open gate, sweep atomicity.
...
- ENABLE_LIVE_TRADING code default RESOLVED by crusaderbot-finalize — config.py default now False (paper-safe); fly.toml [env] still forces false so production posture unchanged. Pending WARP•SENTINEL MAJOR sign-off.
