2026-05-05 21:22 Asia/Jakarta | WARP/CRUSADERBOT-P3B-COPY-TRADE | P3b copy-trade strategy: CopyTradeStrategy (scan: load active copy_targets → wallet_watcher.fetch_recent_wallet_trades 5s timeout + 1 req/s rate limit + swallow on error → 5-min window filter → copy_trade_events dedup → scaler.scale_size proportional + cap + $1 floor → SignalCandidate(confidence=0.75); evaluate_exit: leader-exit detection via fetch_leader_open_condition_ids, encoded as reason='strategy_exit'+metadata.reason='leader_exit' to honour P3a foundation invariant; default_tp_sl=(0.25, 0.10)). New services/copy_trade/{scaler,wallet_watcher}. Migration 009_copy_trade.sql: copy_targets (UNIQUE user_id+wallet) + copy_trade_events (UNIQUE source_tx_hash) — placed at infra/migrations/ per task spec, runner-path divergence carried from P3a. Telegram /copytrade add|remove|list (Tier 2 gate, MAX 3 active rows enforced at handler, 0x+40-hex validator with case-fold, 🗑 Stop callback) + bot/keyboards/copy_trade.py. registry.py extended with bootstrap_default_strategies() (idempotent, lazy-import). main.py lifespan calls bootstrap after init_pool/migrations/cache. dispatcher.py registers /copytrade + ^copytrade: callback. 49 new tests, 245/245 suite green (196 prior + 49 new). No risk gate, execution path, or activation guard touched. Tier: MAJOR. Claim: STRATEGY SIGNAL GENERATION.
