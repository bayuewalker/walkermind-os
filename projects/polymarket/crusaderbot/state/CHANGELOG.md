...
2026-05-07 22:30 Asia/Jakarta | WARP/CRUSADERBOT-P3D-SIGNAL-SCAN-EXECUTION-QUEUE | P3d SENTINEL APPROVED 96/100: side normalization, NO price fix, sub_accounts JOIN removed, subscribe/unsubscribe enrollment wired, migration 012 backfill, zero-value guards. 463/463 tests green. Zero critical issues. WARP🔹CMD merge decision pending.
2026-05-07 20:51 Asia/Jakarta | WARP/CRUSADERBOT-P3D-SIGNAL-SCAN-EXECUTION-QUEUE | P3d FORGE: per-user signal_following scan loop + execution_queue wiring. New services/signal_scan/signal_scan_job.py (run_once, dual-layer dedup, risk gate mandatory), migrations/011_execution_queue.sql (UNIQUE user_id+publication_id), signal_following key added to STRATEGY_AVAILABILITY (supersedes PR #895), scheduler wired. 459/459 tests green. MAJOR. Claim: FULL RUNTIME INTEGRATION (P3d path). Issue #896.
2026-05-07 20:22 Asia/Jakarta | WARP/CRUSADERBOT-P3C-POST-MERGE-SYNC | P3c post-merge state sync: PROJECT_STATE.md, WORKTODO.md, ROADMAP.md updated to reflect PR #892 merged (5ee8487e), SENTINEL APPROVED 100/100. P3d declared as next active MAJOR lane. Issue #893.
2026-05-07 16:30 Asia/Jakarta | WARP/CRUSADERBOT-P3C-SIGNAL-FOLLOWING | P3c signal-following strategy FORGE COMPLETE: SignalFollowingStrategy (BaseStrategy ABC, scan delegates to evaluator, evaluate_exit honours both retire-in-place + separate-exit-row triggers, default_tp_sl=(0.20, 0.08)) + services/signal_feed (idempotent create_feed/publish_signal/publish_exit + advisory-lock-guarded subscribe/unsubscribe + read helpers) + signal_evaluator (pure-DB filter pipeline: blacklist always honoured, categories conservative-skip, $1 floor + capital-pct cap on size, confidence clamp; no HTTP) + /signals Telegram surface (Tier 2 gate, list/catalog/on/off, MAX 5 cap surfaced from service result codes, inline keyboard) + migration 010 (signal_feeds + signal_publications + user_signal_subscriptions, all IF NOT EXISTS, partial UNIQUE on active subscriptions) + bootstrap_default_strategies idempotent across (CopyTrade, SignalFollowing). 67 new tests green. No execution path, no risk gate, no capital logic touched. Refs #882. Tier: MAJOR. Claim: STRATEGY SIGNAL GENERATION. WARP•SENTINEL audit required before merge.
