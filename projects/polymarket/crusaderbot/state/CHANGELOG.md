...
2026-05-05 18:24 | WARP/CRUSADERBOT-R12F-OPERATOR-DASHBOARD | R12f operator dashboard + kill switch + job monitor + audit log — MERGED PR #874. New domain/ops/kill_switch.py (30s cached, fail-safe DB-error returns True), domain/ops/job_tracker.py (APScheduler listener -> job_runs table), migration 007_ops.sql (system_settings, kill_switch_history, job_runs IF NOT EXISTS, idempotent seeds). Bot commands /ops_dashboard, /killswitch, /jobs, /auditlog (operator-only). Risk gate domain/risk/gate.py step [1] uses cached kill_switch module. 51 new tests, 140/140 pass. Tier: STANDARD. Claim: OPERATOR TOOLING ONLY.
