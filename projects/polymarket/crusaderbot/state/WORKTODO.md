...
## Right Now
- [ ] role-model-admin-user — PR open (WARP/role-model-admin-user, 2026-05-17). Two-role model (admin + user): paper opened to every user (risk-gate step-3 scoped to live, setup gate removed); operator/tier/allowlist/premium user-facing wording removed → "Admin access required." / "This feature is not available."; admin.py 🛠 Admin surface + settier user|admin. assert_live_guards (Tier-4 live safety) intentionally preserved per CLAUDE.md (declined to remove). 1413 pytest pass, ruff clean. MAJOR, NARROW INTEGRATION. Awaiting WARP•SENTINEL then WARP🔹CMD merge.
...
- [ ] WARP/full-callback-prefix-migration — F-02+F-03 from sentinel report. Linear tracking item filed under team CrusaderBot. Rewrite 8 remaining keyboard modules to use _common.py helpers + nav:/act:/cfg: namespace; drop legacy patterns from bot/dispatcher.py once all senders migrate.
