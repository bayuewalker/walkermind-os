...
- [ ] WARP•R00T full-system pre-public audit fix campaign — verdict CONDITIONAL (safety/money core APPROVED + fail-closed; user-facing correctness + API hardening + pre-LIVE money-path lanes opened). Lanes:
  - [x] Lane 1/5 WARP/ROOT/mode-status-display [STANDARD/NARROW] MERGED #1453 — Theme #1 phantom-key mode/status display (Telegram mvp settings/autotrade/onboarding + WebTrader TopBar context + /me trading_mode + SettingsPage guard row). 5 source-pin tests; ruff + tsc + vite clean. Report: reports/forge/mode-status-display.md.
  - [x] Lane 2/5 WARP/ROOT/silent-broken-features [MAJOR] — copy-trade monitor naive-datetime dead path (tz-aware fix), withdrawal approve/reject notify TypeError (parse_mode passthrough), frontend error-vs-empty states (SettingsPage spinner→error+Retry, PortfolioPage analytics error vs empty, DashboardPage warn not silent). 4 hermetic tests; ruff + tsc + vite clean. PR open — SENTINEL gate suggested. Report: reports/forge/silent-broken-features.md.
  - [ ] Lane 3/5 WARP/ROOT/api-hardening [MAJOR] — ops dashboard exposure when OPS_SECRET unset, JWT/OPS_SECRET in URL, /autotrade/customize input bounds, /auth rate-limit.
  - [ ] Lane 4/5 WARP/ROOT/tg-callback-routing [STANDARD] — dashboard:* + settings:* sub-routes swallowed by MVP handlers (TP/SL + wallet config broken via /settings).
