...
- [ ] WARP•R00T full-system pre-public audit fix campaign — verdict CONDITIONAL (safety/money core APPROVED + fail-closed; user-facing correctness + API hardening + pre-LIVE money-path lanes opened). Lanes:
  - [x] Lane 1/5 WARP/ROOT/mode-status-display [STANDARD/NARROW] — Theme #1 phantom-key mode/status display (Telegram mvp settings/autotrade/onboarding + WebTrader TopBar context + /me trading_mode + SettingsPage guard row). 5 source-pin tests; ruff + tsc + vite clean. PR open. Report: reports/forge/mode-status-display.md.
  - [ ] Lane 2/5 WARP/ROOT/silent-broken-features [MAJOR] — copy-trade monitor naive-datetime dead path, withdrawal approve/reject notify TypeError, frontend error-vs-empty states.
