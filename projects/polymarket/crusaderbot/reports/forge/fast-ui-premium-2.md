# WARP•FORGE REPORT — fast-ui-premium-2
# Track H: Portfolio Charts + Insights

Branch: WARP/CRUSADERBOT-FAST-UI-PREMIUM-2
Date: 2026-05-11
...
- `services/portfolio_chart.py` — `generate_portfolio_chart(user_id, days)` derives
  ...
  Carry-forward anchor injected at `cutoff_date` when pre-window balance exists,
  covering both the inactive-window case (flat line) and the partial-window case
  (anchor prepended before first in-window entry so lows/highs are correct).
...
- APScheduler cron: Monday 08:00 Asia/Jakarta, `id="weekly_insights"`, `coalesce=True`.
...
- `projects/polymarket/crusaderbot/tests/test_portfolio_charts_insights.py` (29 tests)
- `projects/polymarket/crusaderbot/reports/forge/fast-ui-premium-2.md` (this report)
...
- Carry-forward anchor: funded-but-inactive users see a flat line; users with pre-window
  balance + in-window activity see the correct opening balance at `cutoff_date`.
...
- 29 hermetic tests green (no DB, no network).
...
- WARP✔️CMD review → merge (no SENTINEL required, Tier: STANDARD).
...
**Not in Scope:** AI-generated insights, live price data, DB schema changes, trading logic, activation guards, CLOB/order placement, risk/capital constants
**Suggested Next Step:** WARP✔️CMD review → merge
