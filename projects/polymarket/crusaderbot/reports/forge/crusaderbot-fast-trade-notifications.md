...
**Branch:** WARP/crusaderbot-fast-trade-notifications
**Date:** 2026-05-11
...
**Suggested Next Step:** WARP🔹CMD review; merge when satisfied. Track B (Copy Trade execution) can proceed concurrently.
...
- `projects/polymarket/crusaderbot/bot/handlers/my_trades.py` — added MANUAL notification hook in close_confirm_cb
...
- COPY_TRADE scaffold ready for Track B wiring (target_wallet param accepted but Track B provides it)
- No activation guard mutations — paper mode only
- No threading — asyncio throughout
- Full type hints on all production code
- Structlog for module-level logger; stdlib logging in failure path (avoids structlog `event` kwarg collision)
- No `phase*/` folders
...
- COPY_TRADE `notify_copy_trade_entry()` is a scaffold: it exists in `TradeNotifier` but is not wired into any execution path yet. Track B (Copy Trade execution) will call it when copy_trade positions open.
...
- Track B (Copy Trade execution) — wire `TradeNotifier.notify_copy_trade_entry()` when copy_trade positions are opened by the execution worker
- Track D (Risk caps + kill switch hardening) — MAJOR, SENTINEL REQUIRED
- Track E (Daily P&L report) — STANDARD, can proceed after Track C merge
