...
* **Duration whitelist resolution (WARP🔹CMD Option B applied):** Issue #1269 listed `Duration whitelist: 5m, 15m only` as a Done Criterion, but Polymarket Gamma payloads do not expose a reliable 5m/15m timeframe discriminator (no `duration` field; market `endDate − startDate` does not bucket cleanly into 5m / 15m; slug conventions are inconsistent). The issue's "No new strategy math beyond eligibility wiring" guard rules out inventing one. Per WARP🔹CMD review on PR #1270: claim removed from UI copy and report language; this lane is explicitly **crypto-only eligibility (category=Crypto + BTC/ETH/SOL/XRP/DOGE/BNB/HYPE asset whitelist) + UI metadata**, NOT duration-gated runtime. The WebTrader card now reads `Fast crypto momentum + liquidity confluence (BTC / ETH / SOL / XRP / DOGE / BNB / HYPE only)` — no timeframe claim.
