...
**Not in Scope:** No changes to the execution engine, risk gate, risk constants, activation guards, live trading guard, existing strategy cards, or strategy math beyond eligibility wiring. No runtime duration-gating (Polymarket markets lack a 5m/15m discriminator — see §5; WARP🔹CMD Option B applied to remove the runtime claim). No Telegram UX in this lane (shared preset catalog metadata picked up automatically; no preset picker keyboard reshuffle).
...
* `signal_scan_job.run_once()` flow per user, per tick:
  ...
  2. Fetch raw Gamma market list once for the tick. Hoist the `confluence_scalper` strategy instance once via `StrategyRegistry.instance().get(_CONFLUENCE_SCALPER_NAME)` — the lookup is process-wide and does not depend on per-user data.
  ...
  4. **Phase B (NEW): if `_preset_allows(active_preset, 'confluence_scalper')`** → call `confluence_strat.scan(market_filters, user_ctx)` on the hoisted instance. The strategy applies `is_confluence_scalper_eligible(m)` inside its own market loop (see `ConfluenceScalperStrategy.scan` in `domain/strategy/strategies/confluence_scalper.py`), so emitted candidates already satisfy the asset whitelist when they reach the scan loop. Survivors flow through the same `_process_candidate(row, cand)` path as Phase A. No second-pass ID-set gate in the scan loop (deliberately removed in response to PR #1270 CodeRabbit MAJOR finding — gating with the lib-strategy `limit=200` snapshot could silently drop valid confluence candidates that were not present in the capped lib snapshot).
...
* `projects/polymarket/crusaderbot/services/signal_scan/signal_scan_job.py` — expanded `_PRESET_ALLOWED` with `confluence_scalper` and Full Auto inclusion; hoisted the `StrategyRegistry.instance().get(_CONFLUENCE_SCALPER_NAME)` lookup once per tick (out of the per-user loop) per Gemini PR review; added Phase B domain-strategy execution block inside `run_once()` using the hoisted instance — no second-pass ID-set gating, eligibility now lives in `ConfluenceScalperStrategy.scan()`; extended log + event_bus emit with `confluence_signals` count.
...
* `projects/polymarket/crusaderbot/webtrader/frontend/src/pages/AutoTradePage.tsx` — inserted `confluence_scalper` entry into the `STRATEGY_PRESETS` const array between `ensemble` and `full_auto`; signal copy `Fast crypto momentum (BTC/ETH/SOL/XRP/DOGE/BNB/HYPE)` (shortened to avoid `truncate` clipping per CodeRabbit PR review); engine label `ConfluenceScalperStrategy`; risk `advanced`; freq `High`.
* `projects/polymarket/crusaderbot/domain/strategy/strategies/confluence_scalper.py` — `scan()` now applies `is_confluence_scalper_eligible(m)` inside its own market loop before invoking `_evaluate_market(...)`, so non-crypto / off-whitelist markets self-skip at the strategy boundary. No strategy math change beyond the eligibility filter wiring per Issue #1269 scope.
* `projects/polymarket/crusaderbot/tests/test_confluence_scalper.py` — `_make_market()` helper defaults updated with `category="Crypto"` + `question="Will BTC hit a new high?"` so existing 36 tests stay green against the new eligibility gate; added 3 new tests covering non-crypto self-skip, off-whitelist self-skip, and per-asset (BTC/ETH/SOL/XRP/DOGE/BNB/HYPE) acceptance.

**Created (2):**
* `projects/polymarket/crusaderbot/domain/strategy/eligibility.py` — standalone eligibility module exporting `CONFLUENCE_SCALPER_ASSETS`, `CONFLUENCE_SCALPER_ASSET_PATTERN`, `is_confluence_scalper_eligible(market: Any) -> bool`, and `eligible_market_ids_for_confluence_scalper(markets: Iterable[Mapping[str, Any]]) -> set[str]`. Behavior-identical to the previously-inline regex (verified by replaying all 20 PR test cases through the standalone module). Type hints tightened to use `collections.abc.Iterable[Mapping[str, Any]]` per CodeRabbit PR review.
...
* Pytest exercised post-push via the FORGE container after dependency install — the new test file passes **33/33** (`pytest projects/polymarket/crusaderbot/tests/test_webtrader_confluence_scalper_exposure.py`). The broader `tests/` directory carries 4 pre-existing failures in `tests/test_warp59_copy_wallet_bridge.py::test_do_start_copying_*` / `::test_do_pause_*` / `::test_bridge_end_to_end_*` (`AttributeError: 'types.SimpleNamespace' object has no attribute 'effective_message'` at `bot/handlers/mvp/_send.py:44`). Those failures were merged with WARP-59 (`68a523e`) before any pytest run in CI and are NOT introduced by this lane — out of scope for WARP-61. Recommend a follow-up lane to patch the WARP-59 test fixtures.
...
* **Duration whitelist resolution (WARP🔹CMD Option B applied):** Issue #1269 listed `Duration whitelist: 5m, 15m only` as a Done Criterion, but Polymarket Gamma payloads do not expose a reliable 5m/15m timeframe discriminator (no `duration` field; market `endDate − startDate` does not bucket cleanly into 5m / 15m; slug conventions are inconsistent). The issue's "No new strategy math beyond eligibility wiring" guard rules out inventing one. Per WARP🔹CMD review on PR #1270: claim removed from UI copy and report language; this lane is explicitly **crypto-only eligibility (category=Crypto + BTC/ETH/SOL/XRP/DOGE/BNB/HYPE asset whitelist) + UI metadata**, NOT duration-gated runtime. The WebTrader card now reads `Fast crypto momentum (BTC/ETH/SOL/XRP/DOGE/BNB/HYPE)` — no timeframe claim, short enough to avoid the `truncate` class clipping in the preset card grid.
