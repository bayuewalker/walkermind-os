...
- **Leader bankroll estimate.** The strategy currently treats `target.leader_bankroll_estimate` as optional and falls back to `max(leader_size_usdc, 1.0)` when absent. This conservatively scales mirror size to the leader's *latest trade size* rather than their portfolio — i.e. a 1× mirror at the cap. A real Polymarket-positions-based estimator is deferred to P3c+; the column is not yet on the table because no estimator writes to it. WARP🔹CMD may want to add `leader_bankroll_estimate DOUBLE PRECISION` in a follow-up migration.
