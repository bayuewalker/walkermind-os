...
- **Legacy copy_targets schema co-existence.** `001_init.sql` defines `copy_targets` with a different column set (`wallet_address`, `enabled`, `last_seen_tx`) which `bot/handlers/setup.py` still reads/writes. Migration 009 reconciles the two schemas: it adds the P3b columns idempotently via `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, backfills `target_wallet_address` from `wallet_address` and `status` from `enabled` using guarded `DO` blocks, drops `NOT NULL` on the legacy `wallet_address` so future P3b inserts succeed, and adds the new `UNIQUE (user_id, target_wallet_address)` constraint via a guarded `DO` block on upgraded DBs. Both column families remain on the table — the legacy reader and the P3b reader operate on disjoint subsets of rows. A follow-up lane should retire the legacy `setup.py` copy-target code path and drop the legacy columns; that cleanup is out of scope here.
