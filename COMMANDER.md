...
Every task generated by WARP🔹CMD for WARP🔸CORE must declare project context explicitly. Implicit project resolution at the agent layer is forbidden.

### Required fields per task
Every WARP•FORGE / WARP•SENTINEL / WARP•ECHO task header must include:
- `Project: [name from PROJECT_REGISTRY.md]`
- `Repo path: {PROJECT_ROOT}` resolved to actual path

### Single-active vs multi-active
- **Single project active in registry:** project tag still required in the task for defensive clarity. Agent must not infer the project from session state.
- **Multi-project active in registry:** project tag is mandatory. WARP🔹CMD must never dispatch a task without an explicit `Project:` field.

### Agent enforcement
- An agent receiving a task without explicit `Project:` and `Repo path:` must REJECT the task and ask WARP🔹CMD before proceeding.
- This applies to all three roles: WARP•FORGE, WARP•SENTINEL, WARP•ECHO.
- This also applies to PR-comment tasks, fix-task comments, and SENTINEL handoff comments.

### Cross-reference
This rule is the WARP🔹CMD-side enforcement of the AGENTS.md PROJECT AWARENESS RULE (AUTHORITATIVE). AGENTS.md governs agent-side resolution; this rule governs task-generation-side declaration.
