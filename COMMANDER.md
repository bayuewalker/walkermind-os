...
Note: PROJECT AWARENESS RULE, AGENT IDENTITY VERIFICATION, and ESCALATION MATRIX are now codified as authoritative sections in `AGENTS.md`. Apply the AGENTS.md versions; this file no longer redefines them.
...
## PROJECT CONTEXT IN TASKS (AUTHORITATIVE)

Every task generated by WARP🔹CMD for WARP🔸CORE should declare project context as the default posture. Authority on agent-side rejection lives in AGENTS.md PROJECT AWARENESS RULE; this rule governs WARP🔹CMD-side task-generation discipline.

### Recommended fields per task
WARP•FORGE / WARP•SENTINEL / WARP•ECHO task headers should include:
- `Project: [name from PROJECT_REGISTRY.md]`
- `Repo path: {PROJECT_ROOT}` resolved to actual path

### Single-active vs multi-active
- **Single project active in registry:** project tag is RECOMMENDED for defensive clarity but NOT required — aligns with AGENTS.md PROJECT AWARENESS RULE ("1 project active → no tag required"). Agent does NOT reject a single-active task missing the tag; it resolves project from registry default.
- **Multi-project active in registry:** project tag is MANDATORY. WARP🔹CMD must never dispatch a task without an explicit `Project:` field. Agent MUST reject a multi-active task missing the tag, per AGENTS.md PROJECT AWARENESS RULE.

### Agent enforcement (matches AGENTS.md authority)
- Multi-active + missing tag → agent REJECTS task and asks WARP🔹CMD.
- Single-active + missing tag → agent resolves from registry default; no rejection.
- Tag references a project not in registry, or a `💤 DORMANT` / `⏸️ PAUSED` project → STOP per AGENTS.md PROJECT AWARENESS RULE.
- Same logic applies to PR-comment tasks, fix-task comments, and SENTINEL handoff comments.

### Cross-reference
This rule is the WARP🔹CMD-side discipline reinforcing the AGENTS.md PROJECT AWARENESS RULE (AUTHORITATIVE). On any conflict, AGENTS.md wins per RULE PRIORITY.
